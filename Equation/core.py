...
vmatch = re.compile(r"\s*(?:(?P<oct>(?P<octsign>[+-]?)\s*0o(?P<octvalue>[0-7]+))|"
                    r"(?P<hex>(?P<hexsign>[+-]?)\s*0x(?P<hexvalue>[0-9a-fA-F]+))|"
                    r"(?P<bin>(?P<binsign>[+-]?)\s*0b(?P<binvalue>[01]+))|"
                    r"(?P<dec>(?P<rsign>[+-]?)\s*(?P<rvalue>(?:\d+\.\d+|\d+\.|\.\d+|\d+))"
                    r"(?:[Ee](?P<rexpoent>[+-]?\d+))?(?:\s*(?P<sep>(?(rvalue)\+|))?\s*"
                    r"(?P<isign>(?(rvalue)(?(sep)[+-]?|[+-])|[+-]?)?)\s*"
                    r"(?P<ivalue>(?:\d+\.\d+|\d+\.|\.\d+|\d+))"
                    r"(?:[Ee](?P<iexpoent>[+-]?\d+))?[ij])?))")
